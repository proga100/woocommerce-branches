...
class OrderPackageSplit
{
    ...
    public function woocommerce_admin_order_item_headers($order)
    {
        ...
        <th class="item-th-check">Single item Packing Slip
            <div class="butpdf"><a class="btn btn-success pdf-general" target="_blank" alt="PDF Packing Slip">
        ...
        if ($doc_type == 'packing-slip') {
            ...
            foreach ($data_list as $item_id => $product) {
                if (!empty($_GET['stm_product_id'])) {
                    if ($_GET['stm_product_id'] == $product['product_id']) $modified_data_list[$item_id] = $product;
    ...
    function filter_woocommerce_order_get_items($items, $instance)
    {
        if ($this->doctype == 'packing-slip') {
            ...
            foreach ($items as $item_id => $item_values) {
                if (!empty($_GET['stm_product_id'])) {
                    if ($_GET['stm_product_id'] == $item_values->get_product_id()) $modified_items[$item_id] = $item_values;;
